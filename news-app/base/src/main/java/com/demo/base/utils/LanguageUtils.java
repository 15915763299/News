...
public class LanguageUtils {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isNeedStartActivity) {
        if (isFollowSystem) {
            SpUtils.getBaseSp().put(KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            String localLanguage = locale.getLanguage();
            String localCountry = locale.getCountry();
            SpUtils.getBaseSp().put(KEY_LOCALE, localLanguage + "$" + localCountry);
        }

        updateLanguage(BaseApp.app, locale);
        ...
        if (isNeedStartActivity) {
            ...
            intent.setComponent(new ComponentName(BaseApp.app, realActivityClassName));
    ...
    public static boolean isAppliedSystemLanguage() {
        return VALUE_FOLLOW_SYSTEM.equals(SpUtils.getBaseSp().getString(KEY_LOCALE));
    }
    ...
    public static boolean isAppliedLanguage() {
        return !TextUtils.isEmpty(SpUtils.getBaseSp().getString(KEY_LOCALE));
    }
    ...
    public static Locale getCurrentLocale() {
        return BaseApp.app.getResources().getConfiguration().locale;
    }

    static void applyLanguage(@NonNull final Activity activity) {
        final String spLocale = SpUtils.getBaseSp().getString(KEY_LOCALE);
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            ...
            updateLanguage(BaseApp.app, sysLocale);
        ...
        updateLanguage(BaseApp.app, settingLocale);
    ...
    private static String getLauncherActivity() {
        ...
        intent.setPackage(BaseApp.app.getPackageName());
        PackageManager pm = BaseApp.app.getPackageManager();
